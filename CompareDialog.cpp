...
static const char *shortStructureName[CompareDialog::numStructures] =
{
	"PTV",		// Planning Target Volume
	"rec",
	"bla",
...
const int CompareDialog::xDVHWidget = 256; // Width of DVH display
const int CompareDialog::yDVHWidget = 384; // Height of DVH display
const int xvgaLegendLabelSize = 9;
		...
		matchXYDataExists(true)
{
	...
	gantryAnglePushButton->setFocusPolicy(Qt::ClickFocus);
}
		...
		matchXYDataExists(true)
{
	...
	historyPushButton->setFocusPolicy(Qt::NoFocus);
	removeCurrentMatchPushButton->setFocusPolicy(Qt::NoFocus);
	OKPushButton->setFocusPolicy(Qt::NoFocus);
	cancelPushButton->setFocusPolicy(Qt::NoFocus);
	gantryAnglePushButton->setFocusPolicy(Qt::NoFocus);
	overlaySelectionPushButton->setFocusPolicy(Qt::NoFocus);
	removeSelectedOverlayPushButton->setFocusPolicy(Qt::NoFocus);
}
...
void CompareDialog::createActions()
{
	...
	connect(transparencySpinBox, SIGNAL(valueChanged(int)), this, // was editingFinished()
		SLOT(changeTransparency(int)));
...
#ifdef XVGA_RESOLUTION
			name = "m ";
			name.append(shortStructureName[i]);
#else
			name = "match "
...
/**/
	...
	//chart->GetAxis(vtkAxis::LEFT)->SetTickLabelAlgorithm(
	//	vtkAxis::TICK_WILKINSON_EXTENDED);
}
