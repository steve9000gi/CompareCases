...
// begin SAC 2011/03/10
#include "CaseSpaceDialog.h"
// end SAC 2011/03/11

#include "Patient.h" // SAC 2011/03/24

//-----------------------------------------------------------------------------
class ccChartXYPrivate
{
public:
  ccChartXYPrivate()
    {
	...
	: caseSpaceDialog(NULL)
{
  ...
  this->ChartPrivate = new ccChartXYPrivate;
...
//-----------------------------------------------------------------------------
bool ccChartXY::Paint(vtkContext2D *painter)
{
...
//-----------------------------------------------------------------------------
void ccChartXY::RenderPlots(vtkContext2D *painter)
{
...
//-----------------------------------------------------------------------------
void ccChartXY::CalculateBarPlots()
{
...
//-----------------------------------------------------------------------------
void ccChartXY::RecalculatePlotTransforms()
{
...
//-----------------------------------------------------------------------------
void ccChartXY::RecalculatePlotTransform(vtkAxis *x, vtkAxis *y,
...
//-----------------------------------------------------------------------------
int ccChartXY::GetPlotCorner(vtkPlot *plot)
{
...
//-----------------------------------------------------------------------------
void ccChartXY::SetPlotCorner(vtkPlot *plot, int corner)
{
...
//-----------------------------------------------------------------------------
void ccChartXY::RecalculatePlotBounds()
{
...
//-----------------------------------------------------------------------------
vtkPlot * ccChartXY::AddPlot(int type, int markerStyle /* = vtkPlotPoints::SQUARE */ )
{
  ...
  switch (type)
    ...
    case POINTS:
	  ...
	  // begin SAC 2011/03/09
	  //points->SetMarkerStyle(vtkPlotPoints::SQUARE);
	  points->SetMarkerStyle(markerStyle);
 	  // end SAC 2011/03/09
...
//-----------------------------------------------------------------------------
bool ccChartXY::RemovePlot(vtkIdType index)
{
...
//-----------------------------------------------------------------------------
vtkPlot* ccChartXY::GetPlot(vtkIdType index)
{
  if (static_cast<vtkIdType>(this->ChartPrivate->plots.size()) > index)
    {
...
//-----------------------------------------------------------------------------
vtkIdType ccChartXY::GetNumberOfPlots()
{
  return this->ChartPrivate->plots.size();
}

//-----------------------------------------------------------------------------
vtkAxis* ccChartXY::GetAxis(int axisIndex)
{
  if (axisIndex < 4)
    {
...
//-----------------------------------------------------------------------------
vtkIdType ccChartXY::GetNumberOfAxes()
{
  return 4;
}
...
//-----------------------------------------------------------------------------
void ccChartXY::RecalculateBounds()
{
  // Ensure that the bounds are recalculated
  this->PlotTransformValid = false;
  // Mark the scene as dirty
...
//-----------------------------------------------------------------------------
void ccChartXY::SetScene(vtkContextScene *scene)
{
...
//-----------------------------------------------------------------------------
bool ccChartXY::Hit(const vtkContextMouseEvent &mouse)
{
...
//-----------------------------------------------------------------------------
bool ccChartXY::MouseEnterEvent(const vtkContextMouseEvent &)
{
...
//-----------------------------------------------------------------------------
bool ccChartXY::MouseMoveEvent(const vtkContextMouseEvent &mouse)
{
...
//-----------------------------------------------------------------------------
bool ccChartXY::LocatePointInPlots(const vtkContextMouseEvent &mouse)
{
  size_t n = this->ChartPrivate->plots.size();
  if (mouse.ScreenPos[0] > this->Point1[0] &&
      mouse.ScreenPos[0] < this->Point2[0] &&
        ...
        for (int j = static_cast<int>(this->ChartPrivate->PlotCorners[i].size()-1);
          ...
          if (plot->GetVisible())
            ...
            if (found)
			  ...
			  // begin SAC 2011/03/24
			  // xShift and yShift to display actual values, since everything has
			  // been shifted by - Query Case values in order to put the Query
			  // Case at the origin.  Also add the patient number to the tooltip:

			  float x = plotPos.X();
			  float y = plotPos.Y();

			  Patient *patient = caseSpaceDialog->getDukePatientFromCoodinates(x, y);

			  if (patient) 
			  {
				int pNum = patient->getNumber();
			    ostr << plot->GetLabel() << " #" << pNum <<": " << x + xShift
				     << ", " << y + yShift; 
			  }
			  else
			  {
			    ostr << plot->GetLabel() <<": " << x + xShift
				     << ", " << y + yShift;
			  }
			  // end SAC 2011/03/24
			  ...
			  // begin SAC 2011/03/09
			  // But before returning, also save the selected plot and position:
			  this->selectedPlot = plot;
			  this->selectedPlotPos = plotPos;
			  // end SAC 2011/03/09
...
//-----------------------------------------------------------------------------
bool ccChartXY::MouseLeaveEvent(const vtkContextMouseEvent &)
{
  this->DrawNearestPoint = false;
  this->Tooltip->SetVisible(false);
  return true;
...
//-----------------------------------------------------------------------------
bool ccChartXY::MouseButtonPressEvent(const vtkContextMouseEvent &mouse)
{
  this->Tooltip->SetVisible(false);
  if (mouse.Button == vtkContextMouseEvent::LEFT_BUTTON)
    {
...
//-----------------------------------------------------------------------------
bool ccChartXY::MouseButtonReleaseEvent(const vtkContextMouseEvent &mouse)
{
  // begin SAC 2011/03/09
  if (mouse.Button == vtkContextMouseEvent::LEFT_BUTTON)
  {
    bool found = this->LocatePointInPlots(mouse);

	if (found)
	{
		// color the found point, find which case it belongs to, and make 
		// that info available to the object that <this> belongs to.
        vtksys_ios::ostringstream ostr;
        ostr << "Selected: " << selectedPlot->GetLabel() << ": " << selectedPlotPos.X() << ", " << selectedPlotPos.Y();
		const char *cStr = ostr.str().c_str();
        this->Tooltip->SetText(ostr.str().c_str());

		caseSpaceDialog->setSelectedMatchPlot(selectedPlot);
		caseSpaceDialog->setSelectedMatchPlotPos(&selectedPlotPos);
		caseSpaceDialog->identifyMatchCase();
		return true;
	}
  }
  else
...
void ccChartXY::ZoomInAxes(vtkAxis *x, vtkAxis *y, float *origin, float *max)
{
...
//-----------------------------------------------------------------------------
bool ccChartXY::MouseWheelEvent(const vtkContextMouseEvent &, int delta)
{
  this->Tooltip->SetVisible(false);
...
//-----------------------------------------------------------------------------
void ccChartXY::ProcessSelectionEvent(vtkObject* , void* )
{
}

//-----------------------------------------------------------------------------
bool ccChartXY::RemovePlotFromCorners(vtkPlot *plot)
{
...
//-----------------------------------------------------------------------------
void ccChartXY::PrintSelf(ostream &os, vtkIndent indent)
{
