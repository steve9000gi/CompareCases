...
CaseSpaceDialog::CaseSpaceDialog(MainWindow *mw)
	:	mainWindow(mw),
		...
		queryCaseDataTextActor(NULL),
		matchCaseDataTextActor(NULL)

{
	...
	testPushButton->hide();
	caseDataGroupBox->hide();
    queryCaseLabel->hide();
    queryCaseNameLabel->hide();
    matchCaseLabel->hide();
    matchCaseNameLabel;
...
void CaseSpaceDialog::setBackgroundBlack(bool checked /* = true */ )
{
	if (checked)
	{
		...
		setCaseDataColor(true); // white text
	}
...
void CaseSpaceDialog::setBackgroundWhite(bool checked /* = true */ )
{
	if (checked)
	{
		...
		setCaseDataColor(false); // black text
	}
...
#ifdef XVGA_RESOLUTION
	MILegend->GetPositionCoordinate()->SetValue(0.838, 0.93);
	MILegend->SetWidth(0.101);
	MILegend->SetHeight(0.08);
	int lfs = MILegend->GetLabelTextProperty()->GetFontSize();
	int tfs = MILegend->GetTitleTextProperty()->GetFontSize();
	MILegend->GetLabelTextProperty()->SetFontSize(lfs + 3);
	MILegend->GetTitleTextProperty()->SetFontSize(tfs - 3);
	lfs = MILegend->GetLabelTextProperty()->GetFontSize();
	tfs = MILegend->GetTitleTextProperty()->GetFontSize();
	int xx = 0;
#else
	MILegend->GetPositionCoordinate()->SetValue(0.784, 0.94);
	MILegend->SetWidth(0.164);
	MILegend->SetHeight(0.058);
#endif
...
bool CaseSpaceDialog::readDukeXYData()
{
	...
	{
		in >> n >> XYValue[PTVSize] >> XYValue[rectumSize] >> XYValue[bladderSize] >> XYValue[PTVPlusRectum]
		   >> XYValue[PTVPlusBladder] >> XYValue[PTVPlusRectumPlusBladder];
	...
	for (int i = 0; i < numDukePatients; i++)
	{
		...
		else
		{
			vtkSphereSource *sphere =
				vtkSphereSource::SafeDownCast(dukePoint[i]);

			if (sphere)
			{
				sphere->SetCenter(
					dX[i], dY[i], MIval[queryCaseIndex][i] * zMult);
			}
		}
...
///caseDataTextInit/////////////////////////////////////////////////////////////
//
// Set up text in the upper left hand corner of the window.
//
////////////////////////////////////////////////////////////////////////////////
void CaseSpaceDialog::caseDataTextInit(void)
{
	queryCaseDataTextActor = vtkTextActor::New();
	//queryCaseDataTextActor->GetTextProperty()->SetFontFamilyToCourier();
	queryCaseDataTextActor->SetHeight(0.34);

#ifdef XVGA_RESOLUTION
	queryCaseDataTextActor->SetDisplayPosition(8, 549);
#else
	queryCaseDataTextActor->SetDisplayPosition(10, 790);
#endif
	queryCaseDataTextActor->SetInput("Query: <not selected>");
	ren->AddActor(queryCaseDataTextActor);

	matchCaseDataTextActor = vtkTextActor::New();
	//matchCaseDataTextActor->GetTextProperty()->SetFontFamilyToCourier();
	matchCaseDataTextActor->SetHeight(0.34);

#ifdef XVGA_RESOLUTION
	matchCaseDataTextActor->SetDisplayPosition(8, 533);
#else
	matchCaseDataTextActor->SetDisplayPosition(10, 774);
#endif
	matchCaseDataTextActor->SetInput("Match: <not selected>");
	ren->AddActor(matchCaseDataTextActor);

	setCaseDataColor(true); // make the text white on black
}
...
//
////////////////////////////////////////////////////////////////////////////////
void CaseSpaceDialog::setCaseDataColor(bool makeTextWhite)
{
	QColor txtClr;
	QColor bgClr;
	...
	if (makeTextWhite)
	{
		txtClr.setRgb(255, 255, 255);
		bgClr.setRgb(0, 0, 0, 0);
		caseDataGroupBox->setStyleSheet("QGroupBox { color: solid black; }");
		caseDataGroupBox->setStyleSheet("QGroupBox { border: 2px solid black; }");

		queryCaseDataTextActor->GetTextProperty()->SetColor(1, 1, 1);
		matchCaseDataTextActor->GetTextProperty()->SetColor(1, 1, 1);
	}
	else
	{
		txtClr.setRgb(0, 0, 0);
		bgClr.setRgb(255, 255, 255, 0);
		caseDataGroupBox->setStyleSheet("QGroupBox { color: solid white; }");
		caseDataGroupBox->setStyleSheet("QGroupBox { border: 2px solid white; }");
		queryCaseDataTextActor->GetTextProperty()->SetColor(0, 0, 0);
		matchCaseDataTextActor->GetTextProperty()->SetColor(0, 0, 0);
	}

	queryCaseLabel->setPaletteForegroundColor(txtClr);
	matchCaseLabel->setPaletteForegroundColor(txtClr);
	queryCaseNameLabel->setPaletteForegroundColor(txtClr);
	matchCaseNameLabel->setPaletteForegroundColor(txtClr);

	queryCaseLabel->setPaletteBackgroundColor(bgClr);
	matchCaseLabel->setPaletteBackgroundColor(bgClr);
	queryCaseNameLabel->setPaletteBackgroundColor(bgClr);
	matchCaseNameLabel->setPaletteBackgroundColor(bgClr);


	queryCaseLabel->update();
	matchCaseLabel->update();
	queryCaseNameLabel->update();
	matchCaseNameLabel->update();
...
void CaseSpaceDialog::displayQueryCaseData()
{
	...
	sprintf_s(txt, "Query: %s #%03d: %s: %3g; %s: %3g; MI: %3.3g", 
		...
		getXYValueFromNumber(qNum, xValueType),
		XYValuesItemText[yValueType],
		getXYValueFromNumber(qNum, yValueType),
		MIval[queryCaseIndex][queryCaseIndex]
		);

	ren->AddActor(queryCaseDataTextActor); // Need to do this every time to see text
	queryCaseDataTextActor->SetInput(txt);    
}
...
void CaseSpaceDialog::displayMatchCaseData()
{
	...
	sprintf_s(txt, "Match: %s #%03d: %s: %3g; %s: %3g; MI: %3.3g", 
		...
		matchCase->getNumber(), 
		XYValuesItemText[xValueType],
		getXYValueFromNumber(mNum, xValueType),
		XYValuesItemText[yValueType],
		getXYValueFromNumber(mNum, yValueType),
		MIval[queryCaseIndex][currMatchCaseIndex]
		);

	ren->AddActor(matchCaseDataTextActor); // Need to do this every time to see text
	matchCaseDataTextActor->SetInput(txt);    
}
...
bool CaseSpaceDialog::averageMIData()
{
	...
	int numAngles = 7;	// TEMP: hardwired for Duke configuration
...
void CaseSpaceDialog::prepareMIDisplay()
{
	...
	vtkFollower *xf = axes->AddFollowingText(Axes::xAxis, XYValuesItemText[xValueType], 
		...
		1, 0, 0, scale, ren);
	vtkFollower *yf = axes->AddFollowingText(Axes::yAxis, XYValuesItemText[yValueType],
		queryCase->getPTVPlusBladder(), queryCase->getPTVPlusRectum() + axisTextOffset, z,
		0, 1, 0, scale, ren);
